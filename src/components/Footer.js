        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: "80px", marginBottom: "80px" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "32px" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px" }}>
              {[
                { n: "GitHub", i: <Github size={18} />, h: "https://github.com/PandurangKhavale" },
                { n: "LinkedIn", i: <Linkedin size={18} />, h: "https://www.linkedin.com/in/pandurangkhavale/" },
                { n: "X (Twitter)", i: <Twitter size={18} />, h: "https://x.com/pandurangkhavale" },
                { n: "Email", i: <Mail size={18} />, h: "mailto:pandurangkhavale@outlook.com" },
              ].map((item, i) => (
                <a key={i} href={item.h} target="_blank" rel="noopener noreferrer"
                  style={{ 
                    ...
                    padding: "12px 16px", borderRadius: "14px", border: "1px solid var(--bdr)", background: "var(--bg2)"
                  }}
                  onMouseEnter={e => { e.currentTarget.style.color = "var(--head)"; e.currentTarget.style.borderColor = "var(--cyan)"; e.currentTarget.style.transform = "translateY(-4px)"; onPlayTick(); }}
                  onMouseLeave={e => { e.currentTarget.style.color = "var(--muted)"; e.currentTarget.style.borderColor = "var(--bdr)"; e.currentTarget.style.transform = "translateY(0)"; }}>
                  <span style={{ color: "var(--cyan)" }}>{item.i}</span> {item.n}
                </a>
              ))}
            </div>
          </div>
